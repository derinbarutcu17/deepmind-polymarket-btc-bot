...
from config import (
    SHORT_EMA_PERIOD, LONG_EMA_PERIOD, DRY_RUN,
...
class BTCStrategy:
    def __init__(self, portfolio):
        ...
        self.live_orders: dict[str, str] = {}  # token_id -> order_id
    ...
    def calculate_safe_maker_price(self, best_bid: float, best_ask: float, tick_size=0.01) -> Optional[Decimal]:
        ...
        if spread <= tick_size:
            ...
            limit_price = D(str(best_bid + 0.001)).quantize(TICK, rounding=ROUND_DOWN)
    ...
    async def evaluate_and_execute(
        ...
        for order in list(pending_orders):
            if order.token_id != target_token:
                ...
                if DRY_RUN:
                    ...
                    if order.token_id in self.live_orders:
                        await pm_client.cancel_order(self.live_orders[order.token_id])
                        del self.live_orders[order.token_id]
                continue

            if time.time() - order.timestamp > 1.5:
                if order.action == "BUY" and best_bid > order.limit_price + D("0.005"):
                    logger.info(
                ...
                elif order.action == "SELL" and best_ask < order.limit_price - D("0.005"):
                    logger.info(
        ...
        for pos in existing_positions:
            ...
            if held_bid_d < pos.entry_price * D("0.85"):
                ...
                if DRY_RUN:
                    ...
                    if order_id:
                        self.live_orders[pos.token_id] = order_id
            ...
            # 1c. Take Profit (Maker)
            best_bid_d = D(str(best_bid))
        ...
        if not has_position and not has_pending:
            ...
            if abs(diff) > taker_threshold:
                ...
                trade_size_usd = D(str(min(TRADE_SIZE_USD * 4, MAX_POSITION_USD)))
                logger.info(f"🔥 Sniper Mode: SCALING to ${trade_size_usd}")
            else:
                trade_size_usd = D(str(TRADE_SIZE_USD))
            ...
            if DRY_RUN:
                ...
                if order_id:
                    self.live_orders[target_token] = order_id
        else:
